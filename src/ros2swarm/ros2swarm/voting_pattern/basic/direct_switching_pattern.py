...
class DirectSwitchingPattern(VotingPattern):

    def __init__(self):
        super().__init__('direct_switching_pattern')
        self.declare_parameters(
            namespace='',
            parameters=[
                ('direct_switching_initial_value', 0),
                ('direct_switching_choose_start_value_at_random', True),
                ('direct_switching_min_opinion', 0),
                ('direct_switching_max_opinion', 3),
                ('direct_switching_timer_period', 5.0),
            ])

        self.param_initial_value = self.get_parameter(
            "direct_switching_initial_value").get_parameter_value().integer_value
        self.param_choose_start_value_at_random = self.get_parameter(
            "direct_switching_choose_start_value_at_random").get_parameter_value().bool_value
        self.param_min_opinion = self.get_parameter(
            "direct_switching_min_opinion").get_parameter_value().integer_value
        self.param_max_opinion = self.get_parameter(
            "direct_switching_max_opinion").get_parameter_value().integer_value
        param_timer_period = self.get_parameter(
            "direct_switching_timer_period").get_parameter_value().double_value
    ...
    def direct_switching_broadcast_callback(self, opinion_msg):
...
def main(args=None):
    """Create a node for the direct switching pattern, spins it and handles the destruction."""
    setup_node.init_and_spin(args, DirectSwitchingPattern)
